...
TagHere 테이블 링크(중앙 QR → 테이블 번호 입력) 이용 고객이 주문 전 선택한 성별(남/여)로
"이 매장에 지금 어떤 성별 고객이 앉아 있는지"를 추정해 반환한다.

**집계 방식 (현재 착석 추정):**
- 테이블별로 **가장 최근 일행만** 집계. 같은 테이블에 새 입력이 들어오면 이전 일행은 퇴장한 것으로 간주 (테이블 회전 = 암묵적 퇴장 신호).
- 같은 테이블의 30분 이내 연속 입력은 한 일행으로 묶음 (일행이 각자 폰으로 순차 입력하는 케이스).
- 마지막 입력이 `window_minutes`(최대 체류시간)보다 오래된 테이블은 만료 → 집계 제외.

```json
{ "store_ids": ["...", "..."], "window_minutes": 180 }   // store_ids 최대 100개, window = 최대 체류시간, 기본 180분·최대 1440분
```
→
```json
{ "stats": [{
...
- `window_minutes`는 업종별 평균 체류시간에 맞춰 조절 권장 (주점 120~180분, 식당 60~90분).
