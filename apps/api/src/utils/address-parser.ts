...
export function parseKoreanAddress(address: string): ParsedAddress {
  ...
  // 축약형 → 정식명칭 치환 (긴 것부터 매칭하여 "서울시"가 "서울"보다 먼저 처리)
  const sidoAbbreviations: [RegExp, string][] = [
    [/\b서울특별시\b/, '서울특별시'], // 이미 정식 → 유지
    [/\b부산광역시\b/, '부산광역시'],
    [/\b대구광역시\b/, '대구광역시'],
    [/\b인천광역시\b/, '인천광역시'],
    [/\b광주광역시\b/, '광주광역시'],
  ...
  for (const [pattern, replacement] of sidoAbbreviations) {
...
export function sidoToShort(sido: string | null): string | null {
  if (!sido) return null;
